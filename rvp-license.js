...
function courseRegistration(input) {
  ...
  const packageCourseId = String(input.package_course_id || input.course_id || "").trim().toLowerCase();
  ...
  if (!ID_RE.test(courseId) || !ID_RE.test(packageCourseId) || !title || title.length > 300 || !downloadUrl
      || courseKey.length !== 32 || !/^[a-f0-9]{64}$/.test(packageSha256)) {
    throw new Error("Invalid RVP course registration");
  }
  return { courseId, packageCourseId, title, downloadUrl, courseKey: courseKey.toString("base64"), packageSha256 };
}
...
async function ensureRvpTables(sqlOverride) {
  ...
  const work = (async () => {
    ...
    `;
    await sql`ALTER TABLE rvp_courses ADD COLUMN IF NOT EXISTS package_course_id VARCHAR(80)`;
    await sql`UPDATE rvp_courses SET package_course_id = course_id WHERE package_course_id IS NULL OR package_course_id = ''`;
    await sql`ALTER TABLE rvp_courses ALTER COLUMN package_course_id SET NOT NULL`;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS rvp_courses_package_course_id_idx ON rvp_courses (package_course_id)`;
    await sql`
...
async function registerCourse(input, sqlOverride) {
  ...
  const sql = sqlOverride || db();
  await sql`
    INSERT INTO rvp_courses (course_id, package_course_id, title, download_url, course_key, package_sha256)
    VALUES (${course.courseId}, ${course.packageCourseId}, ${course.title}, ${course.downloadUrl}, ${course.courseKey}, ${course.packageSha256})
    ON CONFLICT (course_id) DO UPDATE
    SET package_course_id = EXCLUDED.package_course_id, title = EXCLUDED.title, download_url = EXCLUDED.download_url,
        course_key = EXCLUDED.course_key, package_sha256 = EXCLUDED.package_sha256, updated_at = NOW()
  `;
  return { courseId: course.courseId, packageCourseId: course.packageCourseId, title: course.title, downloadUrl: course.downloadUrl, packageSha256: course.packageSha256 };
}
...
async function redeem(input, sqlOverride) {
  ...
  const claimed = await sql`
    UPDATE rvp_access_codes AS access
    SET device_id = COALESCE(access.device_id, ${device.deviceId}),
        device_public_key = COALESCE(access.device_public_key, ${device.publicKey.toString("base64")}),
        activated_at = COALESCE(access.activated_at, NOW())
    FROM rvp_courses AS course
    WHERE access.course_id = course.course_id
      AND access.code_hash = ${codeHash(input.code)}
      AND (access.device_id IS NULL OR access.device_id = ${device.deviceId})
      AND (${requestedCourse} = '' OR course.package_course_id = ${requestedCourse})
    RETURNING access.course_id
  `;
  if (!claimed.length) throw new Error("Mã không hợp lệ hoặc đã kích hoạt trên thiết bị khác.");
  const courses = await sql`
    SELECT course_id, package_course_id, title, download_url, course_key, package_sha256
  ...
  return {
    course_id: String(course.package_course_id || course.course_id),
